...
double fractalize(double D, int N, int radial, int symmetry) {
    ...
    for (i=0; i<N; i++) {
        ...
        star[i].rx = star_temp[j][1];
        star[i].ry = star_temp[j][2];
        star[i].rz = star_temp[j][3];
        star[i].vx = star_temp[j][4];
        star[i].vy = star_temp[j][5];
        star[i].vz = star_temp[j][6];
        star_temp[j][0] = 0.0;
    }
...
int generate_m1(int *N, double mlow, double mup, double *M,
    ...
    for (i=0; i<*N; i++) {
        do{
            do {
                xx = drand48();
                if (xx<k1/k2)
                    star[i].mass_epoch = pow(0.5*c1*xx*k2+pow(mlow,c1),1.0/c1);
                else
                    star[i].mass_epoch = pow(c2*(xx*k2-k1)+pow(max(0.5,mlow),c2),1.0/c2);
...
int generate_m2(int an, double *mlim, double *alpha, double Mcl, double M_tmp,
    double *subcount, int *N, double *mmean, double *M,
    ...
    for (i = 0; i < *N; i++) {
        do {
            do {
                ...
                if (alpha[j] != -1.)
                    star[i].mass_epoch = pow(tmp, 1. / (1. + alpha[j]));
                else
...
int generate_m3(int *N,double mlow, double mup, double *M,
...
int generate_m4(int *N,double alpha, double beta, double mu,
    ...
    for (i=0; i<*N; i++) {
        do {
            ...
            star[i].mass_epoch = mu*pow(x,1.0/(1.0-alpha));
...
int generate_plummer(int N, double rtide, double rvir, double D,
    int symmetry){
...
int generate_king(int N, double W0, double *rvir, double *rh,
...
int generate_subr(int N, double S, double rtide, double rvir) {
...
double generate_profile (int N, double Rmax, double Mtot,
...
int get_binaries(int nbin, double M, double rvir, int pairing,
    ...
    for (i=0; i < nbin; i++) {
        ...
        if (!star[2*i][0] || !star[2*i+1][0]) {
            ...
            for (j=2*i+2;j<*N;j++) {
                ...
                star[j-2].mass_epoch = star[j].mass_epoch;
                star[j-2].rx = star[j].rx;
                star[j-2].ry = star[j].ry;
                star[j-2].rz = star[j].rz;
                star[j-2].vx = star[j].vx;
                star[j-2].vy = star[j].vy;
                star[j-2].vz = star[j].vz;
                star[j-2].mass = star[j].mass;
                star[j-2].kstar = star[j].kstar;
                star[j-2].epoch1 = star[j].epoch1;
                star[j-2].ospin = star[j].ospin;
                star[j-2].rstar = star[j].rstar;
                star[j-2].lstar = star[j].lstar;
                star[j-2].epochstar = star[j].epochstar;
            }
...
int order(int N, double M, double msort, int pairing){

    int i,j;
    int Nhighmass;
    star_struct *star_temp;
    star_temp = (star_struct *)calloc(N, sizeof(star_struct));
    ...
    for (i=0;i<N;i++) {
        //star[i].mass_epoch = star_temp[i][0]/M;
        star[i].mass_epoch = star_temp[i][0];
        star[i].rx = star_temp[i][1];
        star[i].ry = star_temp[i][2];
        star[i].rz = star_temp[i][3];
        star[i].vx = star_temp[i][4];
        star[i].vy = star_temp[i][5];
        star[i].vz = star_temp[i][6];
        star[i].mass = star_temp[i][7];
        star[i].kstar = star_temp[i][8];
        star[i].epoch1 = star_temp[i][9];
        star[i].ospin = star_temp[i][10];
        star[i].rstar = star_temp[i][11];
        star[i].lstar = star_temp[i][12];
        star[i].epochstar = star_temp[i][13];
        star[i].Zstar = star_temp[i][14];
    }
    ...
    if (pairing==2)
        segregate(Nhighmass, 0.0);

    return 0;
}

int segregate(int N, double S){
    int i,j;
...
int energy_order(int N, int Nstars){
    int i,j;
...
int randomize(int N){
    ...
    for (i=0;i<N;i++) {//copying randomized to temporary array
            ...
            star_temp[j][0] = star[i].mass_epoch;
            star_temp[j][1] = star[i].rx;
            star_temp[j][2] = star[i].ry;
            star_temp[j][3] = star[i].rz;
            star_temp[j][4] = star[i].vx;
            star_temp[j][5] = star[i].vy;
            star_temp[j][6] = star[i].vz;
            star_temp[j][7] = star[i].mass;
            star_temp[j][8] = star[i].kstar;
            star_temp[j][9] = star[i].epoch1;
            star_temp[j][10] = star[i].ospin;
            star_temp[j][11] = star[i].rstar;
            star_temp[j][12] = star[i].lstar;
            star_temp[j][13] = star[i].epochstar;
            star_temp[j][14] = star[i].Zstar;
    }
...
int radial_profile(int N, double rvir, double M,
...
int cmd(int l, double Rgal, double *abvmag, double *vmag,
...
int output0(char *output, int N, int NNBMAX, double RS0, double dtadj,
    ...
    int gpu, int sse, int seed, double extmass, double extrad,
...
int output3(char *output, int N, double rvir, double rh, double mmean,
    ...
    double Rgal, double extmass, double extrad) {
    ...
    #pragma omp parallel shared(N, star)  private(i, j)
        ...
        for (j=0; j<N; j++) {
            if (j) {
                ...
                gaspot -= star[j].mass_epoch/extrad*extmass/sqrt(1.0+(star[j].rx*
                    star[j].rx+star[j].ry*star[j].ry+star[j].rz*
                    star[j].rz)/(extrad*extrad));
            }
        }
        #ifndef NOOMP
        }
    ...
    for (j=0;j<N;j++) {
        cmd(j, Rgal, &abvmag, &vmag, &BV, &Teff, &dvmag, &dBV);
...
#pragma omp parallel
    ...
    int columns = 15;
    star = (star_struct*)calloc(NMAX,sizeof(star_struct));
    ...
    if (mfunc == 1) {
        ...
        generate_m1(&N, mlow, mup, &M, &mmean, MMAX, Mcl, epoch, Z, Rh,
            remnant);
    } else if (mfunc == 2) {
        ...
        if (!mn){
            ...
            generate_m2(an, mlim, alpha, Mcl, M_tmp, subcount, &N, &mmean, &M,
                MMAX, epoch, Z, Rh, remnant);
        }
    } else if (mfunc == 3) {
        printf("\nMaximum stellar mass set to: %.2f\n",MMAX);
        generate_m3(&N, mlow, mup, &M, &mmean, MMAX, Mcl);
        randomize( N);
    } else if (mfunc == 4) {
        printf("\nMaximum stellar mass set to: %.2f\n", MMAX);
        printf("\nUsing L3 IMF (Maschberger 2012)\n");
        generate_m4(&N, alpha_L3, beta_L3, mu_L3, mlow, mup, &M, &mmean,
            MMAX, Mcl, epoch, Z, Rh, remnant);
    } else {
        printf("\nSetting stellar masses to %.1f solar mass\n", single_mass);
        if (!N) N = Mcl/single_mass;
        for (j=0;j<N;j++) {
            star[j].mass_epoch = single_mass;
            star[j].mass = single_mass;
            star[j].kstar = 0;
            star[j].epoch1 = 0.0;
            star[j].ospin = 0.0;
            star[j].rstar = 0.0;
    ...
    if (profile == 1) {
        printf("\nGenerating King model with parameters: "
            "N = %i\t W0 = %g\t Rh = %.3f\t D = %.2f\n",N, W0, Rh, D);
        generate_king(N, W0, &rvirtemp, &rhtemp, &rking, D, symmetry);
    } else if (profile == 2) {
        N = Nunseg;
        ...
        generate_profile(N, Rmax, M, p, &Rh, D, symmetry);
        printf("\nRh = %.1f pc\n", Rh);
    } else if (profile == -1) {
        printf("\nGenerating fractal distribution with parameters: "
            "N = %i\t Rh = %.3f\t D = %.2f\n", N, Rh, D);
        fractalize(D, N, 0, symmetry);
        rvir = Rh;
    } else {
        ...
        rvir = Rh/0.772764;
        rplummer = Rh/1.305;
        generate_plummer(N, rtide, rvir, D, symmetry);
    }
    ...
    if (!(profile == 2) && (S)) {
        ...
        for (i=0;i<Nunseg;i++) {
            ...
            star[i].rx = star[nrandom][1];
            star[i].ry = star[nrandom][2];
            star[i].rz = star[nrandom][3];
            star[i].vx = star[nrandom][4];
            star[i].vy = star[nrandom][5];
            star[i].vz = star[nrandom][6];
    ...
    if (profile == 0) {
        ...
        radial_profile(N, rvir, M, 0, 0, code, &NNBMAX, &RS0, &Rh2D,
            &Rh3D, NNBMAX_NBODY6);
    ...
    radial_profile(N, rvir, M, create_radial_profile,
    ...
    if ((!fbin) && (!nbin)) {
        ...
        for (j=0;j<N;j++) {
            for (p=0;p<columns;p++) {
                ...
                star[j].mass_epoch = star_temp[j][0];
                star[j].rx = star_temp[j][1];
                star[j].ry = star_temp[j][2];
                star[j].rz = star_temp[j][3];
                star[j].vx = star_temp[j][4];
                star[j].vy = star_temp[j][5];
                star[j].vz = star_temp[j][6];
                star[j].mass = star_temp[j][7];
                star[j].kstar = star_temp[j][8];
                star[j].epoch1 = star_temp[j][9];
                star[j].ospin = star_temp[j][10];
                star[j].rstar = star_temp[j][11];
                star[j].lstar = star_temp[j][12];
                star[j].epochstar = star_temp[j][13];
            }
        }
        ...
        get_binaries(nbin, M, rvir, pairing, &N, adis, amin, amax, Rh,
    ...
    if (code == 0) {
        output0(output, N, NNBMAX, RS0, dtadj, dtout, tcrit, rvir, mmean, tf,
            ...
            dtplot, Z, nbin, Q, RG, VG, rtide, gpu, sse, seed, extmass,
        ...
        output3(output, N, rvir, Rh, mmean, M, epoch, Z, RG, VG, rtide,
            Rgal, extmass, extrad);
    }
    else if (code == 4) {
